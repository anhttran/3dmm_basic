...
bool FaceServices2::rendWFixedShape(cv::Mat colorIm, cv::Mat lms,cv::Vec6d poseCLM, float conf,cv::Mat lmVis, cv::Mat &shape, cv::Mat &tex, string model_file, string lm_file, string pose_file, string refDir){
	float renderParams[RENDER_PARAMS_COUNT];
	float renderParams2[RENDER_PARAMS_COUNT];
	//nTri = 40;
	//nTriHess = 300;
	...
	prepareEdgeDistanceMaps(colorIm);
	//time = (double)cv::getTickCount();
	Mat alpha = cv::Mat::zeros(99,1,CV_32F);
	Mat beta = cv::Mat::zeros(99,1,CV_32F);
	Mat exprW = cv::Mat::zeros(29,1,CV_32F);
	Mat exprW_bk;

	int M = 99;
	loadReference2(refDir, model_file, alpha, beta,M);
	shape = festimator.getShape(alpha);
	tex = festimator.getTexture(beta);
	Mat landModel0 = festimator.getLM(shape,poseCLM(4));
	float bCost, cCost, fCost;
	...
	//shape = festimator.getShape(alpha, exprW);
	//tex = festimator.getTexture(beta);
	//sprintf(text,"%s_wExp.ply",model_file.c_str());
	//write_plyFloat(text,shape,tex,faces);
...
bool FaceServices2::loadReference2(string refDir, string model_file, cv::Mat &alpha, cv::Mat &beta, int &M, bool bySID){
	string fname(model_file);
	size_t sep = model_file.find_last_of("\\/");
	if (sep != std::string::npos)
		fname = model_file.substr(sep + 1, model_file.size() - sep - 1);
	if (bySID){
		int len = fname.find_first_of('_');
		fname = fname.substr(0,len);
		fname = fname + string(".ply");
	}
	fname = refDir + fname;
